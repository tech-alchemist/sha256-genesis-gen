...
int main(int argc, char *argv[])
{
	...
	if((argc-1) < 3)
	{
		fprintf(stderr, "[-] Usage: ./genesisgen [options] <vAlertPubKey> \"<pszTimestamp>\" <nBits>\n");
		fprintf(stderr, "           ( nBits expected in decimal , e.g. 486604799 for 0x1d00ffff )\n");
		return 0;		
	}
	...
	{
		fprintf(stderr, "[-] Timestamp Size dude, Better to keep it length = 1(Min) - 90(Avg) - 254(Max)\n");
		return 0;
	}	
	...
	if(!transaction)
	{
		fprintf(stderr, "[-] memory Allocation, <= Unable to have this thing Exiting...\n");
		return 0;	
	}
	...
	//if(generateBlock)
	{
		...
		while(1)
		{
			...
			{
				printf("\r[/] Generating block => %d Hashes/s, Nonce %u\r", counter, startNonce);
				counter = 0;
				start = time(NULL);
			}
			*pNonce = startNonce;
